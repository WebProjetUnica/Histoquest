...
<div class="game-layout">
  ...
  <aside class="game-panel-left">
    ...
    <!-- Compteur de votes -->
    <p class="vote-count" id="vote-count" role="status" aria-live="polite">0/5 joueurs ont voté</p>
  ...
  <!-- CARTE DU MONDE / ZONE DE JEU -->
  <section class="game-map" id="game-map" aria-label="Carte du monde interactive">
    <div class="map-topbar">
      <div class="round-pill">
        <span class="round-pill__dot"></span>
        Tour <?= (int)$tour ?> / <?= (int)($game['max_tours'] ?? 7) ?>
      </div>
      <div class="timer-pill" aria-live="polite">
        <span class="timer-pill__icon">⏱</span>
        <span id="map-timer-text">30s restantes</span>
      </div>
    </div>

    <div class="world-stage">
      <div class="world-map-visual" aria-hidden="true">
      ...
      <div class="map-choices" id="map-choices">
        <?php foreach ($pays_candidats as $pays): ?>
          <button class="map-choice vote-btn"
                  type="button"
                  data-pays="<?= htmlspecialchars($pays) ?>"
                  aria-label="Voter pour <?= htmlspecialchars($pays) ?>">
            <span class="map-choice__pin">⌖</span>
            <span><?= htmlspecialchars($pays) ?></span>
          </button>
        <?php endforeach; ?>
      </div>

      <div class="map-intel-card">
        <span class="map-intel-card__icon">i</span>
        <div>
          <strong>Analyse d'équipe</strong>
          <p id="map-intel-text">Choisis le pays qui correspond le mieux à l'événement.</p>
        </div>
      </div>
    </div>

    <div class="map-action-bar">
      <button class="btn btn-ghost map-action-btn" id="map-hint-btn" type="button">
        Demander un indice
      </button>
      <button class="btn btn-primary map-action-btn" id="map-focus-btn" type="button" disabled>
        Sélectionne un pays
      </button>
    </div>
  </section>
