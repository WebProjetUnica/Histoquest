function createTimer({ duration, textEl, fillEl, zoneEl, onEnd, onTick }) {
  let secondsLeft = duration;
  let intervalId  = null;

  function updateDisplay() {
    if (textEl) textEl.textContent = secondsLeft;
    if (fillEl) fillEl.style.width = (secondsLeft / duration) * 100 + '%';
    if (zoneEl) {
    ...
    if (typeof onTick === 'function') onTick(secondsLeft, duration);
  }
  ...
  function start() {
    ...
    intervalId = setInterval(() => {
      secondsLeft--;
      updateDisplay();
      if (secondsLeft <= 0) { stop(); if (typeof onEnd === 'function') onEnd(); }
    }, 1000);
  }
